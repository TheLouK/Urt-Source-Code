...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
    ...
    //SV_SendServerCommand(NULL, "print \"%s ^7here ciao\n\"", client->name);
	VM_Call( gvm, GAME_CLIENT_BEGIN, client - svs.clients );
	
	
/*	//team = ps->persistant[3];
	// before update cs with correct team from ps->persistant[3], free red blue spec
	for (i = 0, client_search = svs.clients; i < sv_maxclients->integer ; i++, client_search++) {
		if (client_search->state >= CS_CONNECTED) {
			
			//Cmd_ExecuteString (va("scc all cs \"%d\" \"%s\"", 544 + (client_search - svs.clients), sv.configstrings[544 + clientNum]));
			
			colored_name = client_search->cname;
			index= 544 + i;
			cmdcList = sv.configstrings[index]; 
			
			temp=strdup(cmdcList);
			cmdc[n]=strtok(temp, separator);
			while(cmdc[n] && n<30) cmdc[++n]=strtok(NULL, separator);
			
			cmdc[1]=colored_name;
			strcpy (joinMessage, "");
			//Com_Printf("%s\n",joinMessage);
			for(j=0; j<n; j++) {
				if (Q_stricmp("f1", cmdc[j]) == 0 || Q_stricmp("f2", cmdc[j]) == 0 || Q_stricmp("a0", cmdc[j]) == 0){
					if (Q_stricmp("f0", cmdc[j-1]) == 0 || Q_stricmp("f1", cmdc[j]) == 0 || Q_stricmp("f2", cmdc[j]) == 0){
						strcat(joinMessage,"\\\\");
					} else {
						strcat(joinMessage,"\\");
					}
				} else if (Q_stricmp("n", cmdc[j]) == 0 ){
					//strcpy (joinMessage, "");
				} else{
					strcat(joinMessage,"\\");
				}
				strcat(joinMessage,cmdc[j]);
			}
			free(temp);
			
			Com_Printf("RESENDING CS TO %d %s: %s\n", clientNum, client->cname, joinMessage);
			Cmd_ExecuteString (va("scc all cs \"%d\" \"%s\"", index, joinMessage));
...
void SV_UpdateUserinfo_f( client_t *cl ) {
    ...
    // Save the correct colored name
    Q_strncpyz( cl->cname, Info_ValueForKey (cl->userinfo, "name"), sizeof(cl->cname) );

    if ( sv_coloredNames->integer > 0){  
        colored_name = cl->name;
        ...
        //Com_Printf("++++++++start\n");
        SV_SendServerCommand( NULL, "cs %i \"%s\"\n",index, joinMessage );
        //Com_Printf("CUSTOMClientUserinfoChanged: %d %s\n", cl - svs.clients, joinMessage);
		//SV_SendServerCommand( NULL, "chat \"CHANGEClientUserinfoChanged: cs %i %s\"\n",index, joinMessage );
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
    ...
    playerState_t	*ps, *ps_2;
    client_t        *cl_2;
    int             j;
    char            *joinMessage= malloc(1024);
    char            *message;
    char            *prefix;
	...
	if (clientOK) {
		...
		if ((!u->name) && (sv.state == SS_GAME) && (cl->state == CS_ACTIVE)) {
			Cmd_Args_Sanitize(); //filtra \n e \r 
            
            if (sv_totalMute->integer > 0 && cl->muted && (!Q_stricmp("say", Cmd_Argv(0)) ||
                              !Q_stricmp("say_team", Cmd_Argv(0)) ||
                              !Q_stricmp("tell", Cmd_Argv(0)) ||
                              !Q_stricmp("ut_radio", Cmd_Argv(0)) ||
                              !Q_stricmp("callvote", Cmd_Argv(0)))) {
                SV_SendServerCommand(cl, "print \"You are currently muted\n\"");
                return;
            }
				...
				if (callvoteRequiredConnectTime > 0) {
					...
					if (svs.time - cl->lastConnectTime < callvoteRequiredConnectTime) {
						...
						for (i = 0, clTemp = svs.clients; i < sv_maxclients->integer; i++, clTemp++) {
							...
							if (TEAM_SPECTATOR == atoi(Info_ValueForKey(sv.configstrings[544 + (clTemp - svs.clients)], "t"))) {
								continue;
							}
							playersInGame++;
						}
...
//                if (Q_stricmp("!f ", Cmd_Argv(1)) == 0) {
//                    SV_SendServerCommand(cl, "chat \"^7[^5bot^7]^7: yes sir!\n\"");
//                    return;
//                }
                if ((sv_coloredNames->integer > 0)){
                    ps = SV_GameClientNum(cl - svs.clients);
                    
                    if (ps->persistant[3] == 0) {prefix="^7";} // free
                    if (ps->persistant[3] == 1) {prefix="^1>";} // red
                    if (ps->persistant[3] == 2) {prefix="^4>";} // blue
                    if (ps->persistant[3] == 3) {prefix="^7(SPEC) ";} // spec, broadcasted to all
                    if (!cl->muted){
                        SV_SendServerCommand(NULL, "chat \"%s%s^3: %s\"", prefix, cl->cname, Cmd_Argv(1));
						//cchat "1" ":{Per}:Vega^3: ^3test"
						//Q_snprintf(cmd, sizeof(cmd), "sendclientcommand all cs %i \"\"\n", 544 + i);
						//SV_SendServerCommand(NULL, "scc all \"%s%s^3: %s\"", prefix, cl->name, Cmd_Argv(1));
						//Cmd_ExecuteString (va("scc all cchat \"1\" \"%s^3: %s\"", cl->cname, Cmd_Argv(1)));
                    }
                }
                
			}
            else if (Q_stricmp("say_team", Cmd_Argv(0)) == 0) {
                argsFromOneMaxlen = MAX_SAY_STRLEN;
                if ((sv_coloredNames->integer > 0)){
                    ps = SV_GameClientNum(cl - svs.clients);
                    for (j = 0, cl_2 = svs.clients; j < sv_maxclients->integer; j++, cl_2++){
                        if (cl_2->state != CS_ACTIVE)
                            continue;
                        
                        ps_2 = SV_GameClientNum(j);
                        
                        if (ps->persistant[3] == ps_2->persistant[3]){ 
                            if (ps->persistant[3] == 0) {prefix="^7";} // free
                            if (ps->persistant[3] == 1) {prefix="^1>";} // red
                            if (ps->persistant[3] == 2) {prefix="^4>";} // blue
                            if (ps->persistant[3] == 3) {prefix="^7(SPEC) ";} // spec, broadcasted to team
                            if (!cl->muted){
                                SV_SendServerCommand(cl_2, "chat \"%s%s^3: %s\"", prefix, cl->cname, Cmd_Argv(1));
                            }
                        }
                    }
                }
                ...
                if ((sv_coloredNames->integer > 0)){
                    ps = SV_GameClientNum(cl - svs.clients);
                    cl_2 = SV_GetPlayerByHandle();
                                        
                    if (!cl_2) {
                        SV_SendServerCommand(cl, "chat \"^3~^7> No client found matching %s\n\"", Cmd_Argv(1));
                        //SV_SendServerCommand(cl, "chat \"^3~^7> Correct usage is !pm <player> <message>\n\"");
                        return;
                    }
                    
                    for (j = 2; j<Cmd_Argc(); j++) {
                        strncat(joinMessage,Cmd_Argv(j),1024);
                        strncat(joinMessage," ",1024);
                    }
                    if (ps->persistant[3] == 0) {prefix="";}
                    if (ps->persistant[3] == 1) {prefix="^1>";}
                    if (ps->persistant[3] == 2) {prefix="^4>";}
                    if (ps->persistant[3] == 3) {prefix="";}
                    
                    if (!cl->muted){
                        SV_SendServerCommand(cl, "chat \"%s^3[pm] ^3~^7> ^7%s^3: %s\"", prefix, cl_2->cname, joinMessage);
                    
                        if (cl != cl_2){
                            SV_SendServerCommand(cl_2, "chat \"%s^3[pm] ^7<^3~ ^7%s^3: %s\"", prefix, cl->cname, joinMessage);
                        }
                    }
                }
            ...
            else if (!Q_stricmp("god", Cmd_Argv(0)) ||
                //!Q_stricmp("give", Cmd_Argv(0)) ||
                !Q_stricmp("notarget", Cmd_Argv(0)) ||
                !Q_stricmp("noclip", Cmd_Argv(0)) ||
                !Q_stricmp("levelshot", Cmd_Argv(0)) || 
                !Q_stricmp("forceanim", Cmd_Argv(0)) ||
                !Q_stricmp("setviewpos", Cmd_Argv(0))) {
			...
			if (argsFromOneMaxlen >= 0) {
                ...
                if (Q_stricmp("rr", Cmd_Argv(1)) == 0) {
                    Cmd_TokenizeString("team red");
                }
                else if (Q_stricmp("bb", Cmd_Argv(1)) == 0) {
            ...
            if ((sv_coloredNames->integer > 0)){
                // vm_call in order to see chat in console, but it stop before broadcast it ingame
                if (Q_stricmp("say", Cmd_Argv(0))==0 || Q_stricmp("tell", Cmd_Argv(0))==0 || Q_stricmp("say_team", Cmd_Argv(0))==0){
                    Cvar_Set("sv_disableServerCommand", "1");
                    VM_Call( gvm, GAME_CLIENT_COMMAND, cl - svs.clients );
                    Cvar_Set("sv_disableServerCommand", "0");
                    return;
                }
            }
            // standard vm_call
